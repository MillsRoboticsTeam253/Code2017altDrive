...
public class AltDrive extends Command {
    ...
    protected void execute() {
    	...
    	double turnModifier = (direction + 1.50);
    	
    	//Joystick anti-drift
    	if(Math.abs(throttle) > .125){
    		Robot.drivetraintank.setLeft(throttle * turnModifier);
    	    Robot.drivetraintank.setRight(throttle * (1.50 - turnModifier));
    	} else {
    	...
    	if(Math.abs(throttle) < -.125){
    		...
    		Robot.drivetraintank.setRight_Back(throttle * (1.50 - turnModifier));
    	} else {
